...
class CryptoAPI {
    ...
    async getCoinData(options) {
        try {
            ...
            const res = await 
                axios.get(`${this.baseUrl}?key=${this.apiKey}&ids=${options.coin}&convert=${options.cur}`);

            let output = '';

            if (res.data.length > 1) {
                output = this.multipleCoinsTable(res, options.cur);
            } else {
                output = this.singleCoinTable(res, options.cur, true);
            }
